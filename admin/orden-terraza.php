...
$mysqli = new mysqli('localhost','root','123','bosca');
	...
	if(isset($_POST["submit"])) {
		...
		for($i=0;$i<count($id_ary);$i++) {
			$mysqli->query("UPDATE parrilla SET orden='" . $i . "' WHERE id_parrilla=". $id_ary[$i]);
		}
	}
	
	$mysqli->set_charset("utf8");
	$result = $mysqli->query("SELECT * FROM parrilla ORDER BY orden");
			...
			<div class="row">
				<div class="col-md-12">
					<h3 class="text-center bread-back">
						<?php
						echo "<a class=\"bread\" href=\"index.php\">Inicio</a> - <a class=\"bread\" href=\"parrilla-home.php\">Tipo de producto: Calefacción</a> - Ordenar productos terraza / parrilla";
						?>
					</h3>
				...
				<div class="col-md-12">
					<form class="bread-back" name="frmQA" method="POST" />
						...
						<ul id="sortable-row">
							...
							?>
							<li id=<?php echo $row["id_parrilla"]; ?>><?php echo $row["modelo"]; ?></li>
							<?php 
							}
							$result->free();
							$mysqli->close();  
